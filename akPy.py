...
def sortDict(D,sortKey):
    ...
    isort = np.argsort(D[sortKey],kind='stable')
    for key in D:
...
def plot_granules(D,grd):
    ...
    mngr.window.setGeometry(50,50,1700,1000)      
...
def sw_dist_km(lat,lon):
    ...
    latRad = np.absolute(lat * math.pi / 180)
    dep    = np.cos( 0.5*(latRad[ind+1]+latRad[ind]) ) * dlon 
    dlat = np.diff(lat)
    dist = 111.12*np.sqrt(dlat*dlat + dep * dep)
    # note: 6370*math.pi/180 = 111.17; using 111.12 as in sea water routines
...
def plotGridBoundary(ax,lon,lat):
